...
controladorUsuario.eliminar =async(req,res)=>{////---revisar
    try {
        ...
        if (bdpassword.length>0) {
            ...
            res.status(400).json('No se encontro ningun correo que cincida con el que proporcionaste.')
        }
...
controladorUsuario.actualizarContrasena=async(req,res)=>{
    try {
        ...
        if (bdpassword.length>0) {
            ...
            res.status(400).json({
                message: 'No se encontro una contraseña ligada al correo que proporcionaste'
            })
        }
