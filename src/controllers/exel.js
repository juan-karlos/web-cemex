...
for (let i = 0; i <datos.length; i++) {
    ...
    if (!nombresUnicos.has(nombrePlanta)) {
        ...
        // console.log(nombre_planta, segmento, zona, estado, Porcentaje_cumplimiento,'Fija','movil');

        // Agrega el nombre de planta al conjunto para evitar duplicados
...
for (let i = 0; i < datos.length; i++) {
    ...
    if (!nombresUnicos.has(nombrerec)) {
        ...
        // console.log(nombre_requerimiento,peso,siglas,impacto);

        // Agrega el nombre de planta al conjunto para evitar duplicados
        nombresUnicos.add(nombrerec);

        await pool.query(`INSERT INTO requerimiento (nombre_requerimiento, peso,impacto,siglas)VALUES(?,?,?,?)`,[nombre_requerimiento,peso,impacto,siglas])
...
for (let i = 0; i < datos.length; i++) {
    ...
    // console.log(fech1 || null, fech2 || null, observaciones, estatus, validez_unica, nombre_planta, nombre_requerimiento);
     await pool.query(sqlQuery, [fech1 || null, fech2 || null, observaciones, estatus, null, validez_unica, nombre_planta, nombre_requerimiento]);
}
