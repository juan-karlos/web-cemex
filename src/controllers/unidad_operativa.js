...
controllerPlanta.obtenerPlanta = async (req, res) => {
  ...
  try {
    const [infoPlanta] = await pool.query(
      "Select * FROM unidad_operativa WHERE id_planta =?",
      [idrecu]
    );
    if (infoPlanta != "") res.json(infoPlanta);
    else {
      res.status(404).json({ message: "no se encuentra la planta registrada" });
    }
  } catch (Excepcion) {
...
controllerPlanta.activasFijas = async (req, res) => {
  [plantas] = [];
  try {
    [plantas] = await pool.query(
      "SELECT * from unidad_operativa where activo= true and fija =true"
    );
    res.json(plantas);
  } catch (exepcion) {
    console.log(exepcion);
...
controllerPlanta.inactivasFijas = async (req, res) => {
  try {
    [plantas] = await pool.query(
      "Select * FROM unidad_operativa WHERE activo = false and fija =true"
    );
    res.json(plantas);
  } catch (exepcion) {
    console.log(exepcion);
...
controllerPlanta.activasMoviles = async (req, res) => {
  try {
    [plantas] = await pool.query(
      "SELECT * FROM  unidad_operativa Where activo = true and fija =false"
    );
    res.json(plantas);
  } catch (exepcion) {
    console.log(exepcion);
...
controllerPlanta.inactivasMoviles = async (req, res) => {
  try {
    [plantas] = await pool.query(
      "SELECT * FROM  unidad_operativa WHERE activo = false and fija = false"
    );
    res.json(plantas);
  } catch (exepcion) {
    console.log(exepcion);
...
controllerPlanta.insertPlanta = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      message: "Estas intentando insertar una planta que ya esta registrada",
    });
  }
};
...
controllerPlanta.actualizar = async (req, res) => {
  ...
  try {
    const [infoPlanta] = await pool.query(
      `Select id_planta FROM unidad_operativa WHERE id_planta =?`,
      [ids]
    );
    if (infoPlanta != "") {
      // console.log(ids)
...
controllerPlanta.eliminar = async (req, res) => {
  ...
  try {
    if (infoPlanta != "") {
      await pool.query("DELETE FROM unidad_operativa WHERE id_planta=?", [
        idrecu,
      ]);
